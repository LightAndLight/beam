...
import           Database.Beam.Postgres hiding (Pg, runBeamPostgres)
import           Database.Beam.Postgres.Trans
...
testReturningMany getConn = testCase "runReturningMany (batching via cursor) works" $
 ...
 where
  ...
  runSelectReturningMany ::
    (MonadIO m, FromBackendRow Postgres x) =>
    SqlSelect Postgres x -> (PgT m (Maybe x) -> PgT m a) -> PgT m a
